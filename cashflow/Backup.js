...
//       if (!sheet.getRange("I" + row).isBlank() || sheet.getRange("B" + row).isBlank() || row < transFirstRow) {
//         return;
//       }
...
//       var cellDate = moment(sheet.getRange("B" + row).getValue(), "DD/MM/YYYY");
//       var cellDateStartOfMonth = cellDate.startOf("month");

//       if (cellDateStartOfMonth.diff(currentDate, "month") == 0 && !category.includes("Account Transfer")) {
//         copyRangeToCurrentTransactions(sheet, currentTransactionsSheet, row);
//       }
      
//       handleDebitCardTransactions(sheet, row);

//       // ✅ Call updateShowHideColumn after updating allTransactionsSheet
...
//   var status = sheet.getRange("H"+row).getValue()
//   var lastRow = previousTransactionsLastRow(sheet)
...
//   var sourceValues = sourceRange.getValues();
//   var prvsTransRowToEdit = previousTransactionsLastRow(toSheet);
...
//       setRecurringTransactionProperties(currentTransactionsSheet, transLastRow, currentDate, amount, category, account, description);
...
//     var rowRange = currentTransactionsSheet.getRange("B"+row+":I"+row);
...
// function transactionsLastRow(currentTransactionsSheet){
...
//   }
//   var count = 0;
//   for(var i = lastRow; currentTransactionsSheet.getRange("B"+i).getValue() != ""; i++){
//     count++;
//   }
...
//   var transLastRow = transactionsLastRow(currentTransactionsSheet);
//   var allTransactionLastRow = previousTransactionsLastRow(allTransactionsSheet); //the last row is saved within the function so there is no use to the returning variable
//   var transactionsRange = `B${Number(props.getProperty('transFirstRow'))}:I${transLastRow}`;
//   var previousTransactionsRange = `B${Number(props.getProperty('transFirstRow'))}:I${getPreviousTransactionLastRow()}`;
...
//   var sheet = SpreadsheetApp.getActiveSpreadsheet();
//   var currentTransactionsSheet = sheet.getSheetByName(props.getProperty('currentTransactionsSheet'));
//   var txFirstRow = Number(props.getProperty("transFirstRow"));
//   for(var i = txFirstRow; !currentTransactionsSheet.getRange("B"+i).isBlank(); i++){
//     var description = currentTransactionsSheet.getRange("G"+i).getValue().toString();
//     var amountCell = currentTransactionsSheet.getRange("C"+i)
...
//   netWorthSheet.getRange(netWorthLastRow++, 2,1,4).setValues([[currentDate,stocksWorth + stocksAccountCashSheqel + stocksAccountCashUSDConverted,'Stocks Portfolio Account', 'meitav dash current value + payoneer stock total value']])
...
//     var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Transactions"); // Adjust sheet name
//     var filter = sheet.getFilter();
    
//     if (!filter) {
//         sheet.getRange("J8").activate(); // Adjust column J header if needed
//         sheet.getRange("J8").createFilter();
//         filter = sheet.getFilter();
//     }

//     var criteria = SpreadsheetApp.newFilterCriteria()
//         .setHiddenValues(["Hide"]) // Automatically hides "Hide" rows
//         .build();
    
//     filter.setColumnFilterCriteria(10, criteria); // Column J (adjust if needed)
// }
